...
# df = df.drop_duplicates(['code', 'date'], keep='first')
...
print(f"""
        [종목 중앙값과 비교]
         중앙값의 50배 초과: {(df['close'] > med * 50).sum()} 
         중앙값의 5% 미만 : {(df['close'] < med * 0.05).sum()}
...
# IQR로 이상치 탐지에 한계가 있음
#   하한선이 음수가 나올수가 없는데 규칙을 적용하니 음수값이 확인됨

# 중앙값 5% 미만
tiny = df['close'] < med * 0.05
print(f" 중앙값 5% 미만 : {tiny.sum()}건")
print(f" 이중에 전체 IQR에도 속하는 건수 {(tiny & out_mask).sum()}")
print(f" 중앙값 5% 미만 또는 IQR에도 속하는건수: {(tiny | out_mask).sum()}")
...
print(f"거래량이 음수인 경우: {result.sum()}")
...
print(f"종가 결측 수 : {df['close'].isna().sum()}")
