...
# left.merger(right, on='..', how='..', suffixes=('_left', '_right'))

m3 = p2.merge(companies[['code', 'name']], on='code', suffixes=('_price', '_company'))
print(m3)

# 간혹, merge에서 키가 안맞는 경우..
...
print(" === 공백 체크 ===")
print(f"앞 뒤 공백 : {blk_edge}")
print(f"중간 공백 : {blk_mid}")
...
fw = raw_comp[raw_comp['name'].map(hasfullwidth)]
print(f" === 전각 문자 체크 ===\n {len(fw)}건")
print(fw)

# 4. 타입 불일치
c1 = companies.copy()
c1['code_num'] = c1['code'].str.replace('G','').astype(int)
...
# 기분 열의 타입이 다르면 merge 시 오류가 발생함!!
print("=" * 60)

# outer merge 를 활용하여 매칭이 되지 않는 데이터를 확인할 수 있게해줌
