...
class Account:
    ...
    # 클래스 메소드 : 클래스 자체를 다룸. 첫번째 매개 변수 cls
    @classmethod
    ...
    # 정적 메소드 : 객체, 클래스오 무관한 기능을 담당하는 메소드(유틸리티). @staticmethod 지정
    @staticmethod
    def is_valid_amount(amount):
        return amount >= Account.MIN_DEPOSIT

acc = Account('최재욱', 10000)
print(f"deposit --> {acc.deposit(3000)}")       # 인스턴스 메소드 호출

# 클래스 메소드 호출
acc2 = Account.from_dict({"owner" : "강수진", "balance" : 10000})
...
# 데이터 구조가 변경되거나 메소드를 수정하는 경우 새로 정의해서 대응 가능
