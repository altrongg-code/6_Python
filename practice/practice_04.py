...
new_arr5 = np.where(arr5 % 2 == 1, arr5+10, arr5)
...
arr6 = np.array(list6).round().astype(int)
print(arr6)
...
"""
prices = load_utils.load_one_stock(0)
dates = load_utils.load_dates()
max_price = prices.max()
min_price = prices.min()
max_price_date = dates[prices.argmax()]
min_price_date = dates[prices.argmin()]
print((max_price, max_price_date, min_price, min_price_date))
...
"""
matrix = load_utils.load_matrix()   # (120, 750)
mean_by_code = matrix.mean(axis=1)  # (120,)
mean_by_date = matrix.mean(axis=0)  # (750,)

difference_matrix = matrix - matrix.mean(axis=1,keepdims=True)
